...
def load_article(slug):
    ...
    canonical_url = f"https://joshclaw-sys.github.io/ai-tools-hub/articles/{slug}.html"
...
def publish_medium(article):
    ...
    html_content = f'<h2>Why trust this guide?</h2><p>{meta["description"]}</p>{article["body_html"]}<hr><p><em>Originally published at <a href="{article["canonical_url"]}">ai-tools-hub.example</a></em></p>'
...
def publish_devto(article):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "AIToolsHubBot/1.0 (publishing helper; contact@ai-tools-hub.example)",
        },
        method="POST",
    )
    try:
